...
final class TestTrustManager: ServerTrustManager, @unchecked Sendable {
...
public struct TestAPISession: TokenAPISession {
    let baseURL: URL
    let loginType: String?
    let notificationCenter: NotificationCenter
    let trustManager: TestTrustManager = TestTrustManager(
        evaluators: [:]
    )
